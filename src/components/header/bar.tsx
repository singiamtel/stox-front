...
const BarItem = ({stockSymbol, stockName, price, changePer, changeVol}: StockInfo) => {
	const inc = parseFloat(changeVol) > 0
	return (
		<div className="featured-item">
			<div className="featured-item-name">
				<NavLink className="no-style-link" to={"/stock/" + stockSymbol}> { stockSymbol || "" } </NavLink>
			</div>
			<div className="featured-item-value">
				{ price || "" }
			</div>
...
const Bar = () => {
	...
	async function redirectStock(e:React.FormEvent){
		...
		if(stock){
			try{
				await axios.get( process.env.REACT_APP_API_URL + "/stock/" + stock.toUpperCase()).then((res) => {
					if(res.data === null){
	...
	if(getUsername()){
		text = <div className="conectarse-text"> Bienvenido, {username.charAt(0).toUpperCase() + username.slice(1)} </div>
	}
	else{
  ...
  useEffect(() =>{
    ...
    featuredStocks.forEach((i)=>{
      axios.get( process.env.REACT_APP_API_URL + "/stock/" + i).then((result) =>{
			...
			<div className="header">
				...
				<div className="white-bar">
					...
					<div className="nav-links">
						...
						<div className="nav-links-el">
							<NavLink className={`nav-item ${path === "/wallet" ? "nav-current" : ""}`} to="/wallet" >Mi cartera</NavLink>
						</div>

					</div>
					<div className="search">
						<div className="search-text">
							<form onSubmit={redirectStock} className="search-form" acceptCharset="utf-8">
								<div style={{display:"flex"}}>
									<input type="text" name="stock" className="search-text" value={stock} onChange={(e) => { setStock(e.target.value) }}/>
									<button type="submit" value="" className="submit-search">
										<div className="search-icon">
											<svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
												<path d="M12.5 11H11.71L11.43 10.73C12.41 9.59 13 8.11 13 6.5C13 2.91 10.09 0 6.5 0C2.91 0 0 2.91 0 6.5C0 10.09 2.91 13 6.5 13C8.11 13 9.59 12.41 10.73 11.43L11 11.71V12.5L16 17.49L17.49 16L12.5 11ZM6.5 11C4.01 11 2 8.99 2 6.5C2 4.01 4.01 2 6.5 2C8.99 2 11 4.01 11 6.5C11 8.99 8.99 11 6.5 11Z" fill="#192124"/>
											</svg>
