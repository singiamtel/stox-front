...
function registerRedirect(history : any){
...
function Login({setToken}:any) {
	...
	const handleSubmit = async (e:any) => {
		...
		if(token.status === "success"){
			...
			setStatusText("Success, redirecting to home...")
		}
		else{
			setStatusColor("#ffcccb")
			setStatusText("Error, username or password is wrong")
		}
	...
	return (
		<div className="login-comp">
			<div className="login-button" onClick={() => {registerRedirect(history)}}>
				Registro
			</div>
			...
			<h1>Login</h1>
			<form className="login-form" onSubmit={handleSubmit} acceptCharset="utf-8">
				<span className="login-prompt">Nombre de usuario</span>
				<input className="user-input login-input" type="text" name="username" id="user" onChange={(e:any) => setUserName(e.target.value)}/>
				<span className="login-prompt">Contraseña</span>
				<input className="pass-input login-input" type="password" name="password" id="pass" onChange={(e:any) => setPassword(e.target.value)}/>
