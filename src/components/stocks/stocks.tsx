...
const Stock = ({ stockInfo }: { stockInfo: StockInfo }) => {
  return (
    <div className="stock">
      <div className="stock-left">
        <NavLink
          className="stock-stockSymbol"
          to={"/stock/" + stockInfo.stockSymbol.toUpperCase()}
        >
          {stockInfo.stockSymbol}
        </NavLink>
        <div className="stock-stockName">{stockInfo.stockName}</div>
      </div>
      <div className="stock-middle">{stockInfo.price}</div>
  ...
  useEffect(() => {
    const fetchStocks = async (stocks: string[]) => {
      ...
      for (let el of stocks) {
        await axios
          .get(process.env.REACT_APP_API_URL + "/stock/" + el)
          .then((result: any) => {
