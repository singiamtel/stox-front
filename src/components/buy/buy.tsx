...
async function buyStock(token:any, stock:any, amount : any) {
	...
	return axios.post(process.env.REACT_APP_API_URL + '/buy/'+stock.trim(),JSON.stringify(request), {headers :{
		"Authorization": `Bearer ${token}`, "Content-Type" : "application/json"} })
}

async function sellStock(token:any, stock:any, amount : any) {
	const request = {"token":token.token, "stock":stock, "amount":amount}
	return axios.post( process.env.REACT_APP_API_URL + '/sell/'+stock.trim(),JSON.stringify(request), {headers :{
		"Authorization": `Bearer ${token}`, "Content-Type" : "application/json"} })
}
  ...
  // Placeholder values, gets overwritten when fetchStock returns
	...
	const handleSubmit = async (e:any) => {
		...
		if(status.data.status === "success"){
			...
			setStatusText(`Succesfully ${buy} ${amount} stocks of ${symbol.toUpperCase()}`)
		}
		else{
	...
	useEffect(()=>{
		const fetchStock = async () => {
			await axios.get(process.env.REACT_APP_API_URL + "/stock/" + symbol.toUpperCase()).then((res)=>{
	...
	return (
		<div className="buy">
			<div className="buy-title"> Compra y Venta </div>
			...
			<div className="buy-body">
				...
				<div className="buy-bodyForm">
					<div onClick={triggerToggle} className='toggle'>
						<div className={`toggle-button ${toggle ? "toggle-checked" : "toggle-unchecked"} `}>
							Buy
						</div>
						<div className={`toggle-button ${toggle ? "toggle-unchecked" : "toggle-checked"} `}>
							Sell
						</div>
					</div>
					<form onSubmit={handleSubmit}>
						<input type="text" placeholder="volumen" onChange={(e:any) => setAmount(e.target.value)}/>
						<button>Submit </button>
					</form>
				</div>
			</div>
			<div className="api-response-buy" style={{backgroundColor: statusColor}}>
				{statusText}
			</div>
		</div>
	);
